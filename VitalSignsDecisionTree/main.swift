...
func run() {
    ...
    let tree = DecisionTreeCreator(dataElements: Array(trainDataElements[0..<1350])).execute()
    printTree(tree)
    print("\n")
    
    var answers: [(id: Int, severityClass: SeverityClass)] = []
    
    for testDataElement in Array(testDataElements[1350..<1500]) {
    ...
    let correctPercentage = Double(correctCount) / Double(answers.count)
